...
class JevPlatformTests(unittest.TestCase):
    def test_windows_requires_env_or_native_credential_storage(self):
        path = Path("unused")
        with mock.patch.object(LAUNCHER.os, "name", "nt"), mock.patch.object(LAUNCHER.os, "open") as open_file:
            with self.assertRaisesRegex(ValueError, "POSIX"):
                LAUNCHER.load_jev_env(path, {})
            environment = {"JEV_API_KEY": "synthetic-process"}
            self.assertEqual(LAUNCHER.load_jev_env(path, environment), environment)
