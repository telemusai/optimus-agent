...
pub const NODE_TOOL_PROMPT: &str = "Execution mode: Node. Use the `node` tool to execute JavaScript in a persistent Node.js workspace. JavaScript is the orchestration language in this mode; send source in the code parameter, not Python or Markdown fences. Top-level await works, and const/let/var bindings and functions persist between cells. Use require(\"node:fs/promises\") for files, require(\"node:child_process\") for commands, and require(\"node:util\").promisify for asynchronous execFile. Prefer rg for code search. For ESM use await nodeImport(\"module-or-path\"); direct import declarations and import() are not supported by this evaluator. Await work needed for the current result; set bounded command timeouts and avoid detached processes. Console output and the final expression are displayed. A cancelled or timed-out cell resets the JavaScript workspace, including its variables; ordinary errors do not. Variables survive live mode switches but not daemon restarts or session reopening. Python cells and Python-only skills, rlm, and kernel MCP objects are unavailable here; switch to IPython for those integrations. Other explicitly exposed tools, including Jev decisions, remain available. Prior tool calls in history belong to their previous modes: follow the current schemas for new actions. Keep progress updates and final answers in prose.";
...
pub fn build_rlm_prompt(options: &RlmPromptOptions) -> String {
    ...
    } else if active_tools.iter().any(|tool| tool == "node") {
        parts.push(NODE_TOOL_PROMPT.to_string());
