...
async fn fixture() -> (Arc<AgentSession>, tempfile::TempDir) {
    ...
    *session.session_manager.lock().unwrap() =
        SessionManager::create(&session.cwd, Some(&root.path().to_string_lossy())).unwrap();
    (session, root)
}
...
fn child_report(id: &str) -> CustomMessage {
    CustomMessage {
        ...
        details: Some(
            serde_json::json!({"id":id,"message":format!("child evidence {id}"),"fromRelationship":"child","fromName":"fixture"}),
        ),
        display: true,
        timestamp: 1000,
    }
...
fn family_message(id: &str, relationship: &str) -> CustomMessage {
    ...
    message.details.as_mut().unwrap()["message"] =
        format!("{relationship} instruction {id}").into();
    message
}
...
fn enqueue(
...
) -> (QueuedSessionAction, Arc<ActionTicketController>) {
    let agent_message_id = custom
        .as_ref()
    ...
    let (_, ticket, _) = session
        .admit_session_input_with_options(action.clone(), false, false, false, false)
        .unwrap();
...
async fn pump(session: &Arc<AgentSession>) {
    tokio::time::timeout(
        ...
        session.pump_session_inputs(session.session_input_pump_epoch.load(Ordering::SeqCst)),
    )
    .await
...
async fn durable_receipts_survive_compaction_context_replacement_and_reopen() {
    ...
    let (child, child_ticket) = enqueue(
        &session,
        "child durable fixture",
        Some(child_report("durable-child")),
    );
    ...
    session.agent.set_stream_fn(Arc::new(move |model, _, _| {
        ...
        Box::pin(async move {
            ...
            session
                ...
                .unwrap()
                .append_compaction(
                    "earlier work retained in summary",
                    &last_id,
                    9999.0,
                    None,
                    None,
            ...
            session
                .agent
                .update_state(Box::new(move |state| state.messages = context.messages));
            let live = session.messages();
            assert!(
                originals.iter().all(|action| {
                    !live.contains(&agent_message_from_delivery(
                        &primary_delivery_record(action).unwrap().message,
                    ))
                }),
    ...
    assert_eq!(
        ...
        1,
        "no replay to repair a stale snapshot"
    );
    let path = session
        .session_manager
        .lock()
    ...
    assert_eq!(
        count_input(&reopened.get_entries(), "human durable fixture"),
        1
    );
    assert_eq!(
        count_input(&reopened.get_entries(), "child evidence durable-child"),
        1
    );
...
async fn delivery_is_durable_before_model_execution_even_when_event_processing_is_delayed() {
    ...
    session.push_agent_event_task(async move {
    ...
    session
        ...
        .update_action(&selected)
        .unwrap();
    session.agent.update_state(Box::new({
        let message = message.clone();
        move |state| state.messages.push(message)
    }));
    session.handle_agent_event(AgentEvent::MessageStart {
        message: message.clone(),
    });
    session.handle_agent_event(AgentEvent::MessageEnd { message });
    ticket.ticket.delivered.clone().await.unwrap();
    assert!(session
        .settled_turn_delivery_error(&[action.clone()])
        .is_none());
    let path = session
        .session_manager
        .lock()
    ...
    let reopened = SessionManager::open(&path, None, None).unwrap();
    assert_eq!(
        count_input(&reopened.get_entries(), "delayed durable event"),
        1
    );
    release.cancel();
    session.await_agent_event_queue().await;
    ...
    assert_eq!(
        count_input(
            &session.session_manager.lock().unwrap().get_entries(),
            "delayed durable event"
        ),
        1
    );
    session.dispose_async(Some(false)).await;
...
async fn failed_transcript_write_rejects_delivery_and_retains_input_for_flush_recovery() {
    ...
    assert!(
        failure.contains("transcript persistence failed"),
        "{failure}"
    );
    ...
    assert_eq!(
        calls.load(Ordering::SeqCst),
        0,
        "failed initial persistence must prevent provider/tool work"
    );
    std::fs::remove_dir(&path).unwrap();
    session.session_manager.lock().unwrap().flush_now().unwrap();
    let reopened = SessionManager::open(&path, None, None).unwrap();
    assert_eq!(
        count_input(&reopened.get_entries(), "recover-unsaved-input"),
        1
    );
    assert!(
        !session.has_failed_dispatch_persistence(),
        "settled failure must not suppress a later independent turn"
    );
    session
        .prompt("new task after persistence repair", None)
        .await
        .unwrap();
    assert_eq!(
        calls.load(Ordering::SeqCst),
        1,
        "a new explicit task runs normally without replaying the failed action"
    );
    ...
    std::fs::create_dir(&path).unwrap();
    assert!(session
        .prompt("unsaved-after-flushed", None)
        .await
        .unwrap_err()
    ...
    let reopened = SessionManager::open(&path, None, None).unwrap();
    assert_eq!(
        count_input(&reopened.get_entries(), "unsaved-after-flushed"),
        1
    );
    session.dispose_async(Some(false)).await;
}
...
async fn child_backlog_batches_once_while_human_order_and_one_at_a_time_are_preserved() {
    ...
    session
        ...
        .set_stream_fn(Arc::new(move |model, context, _| {
            seen.lock()
                .unwrap()
                .push(serde_json::to_string(&context.messages).unwrap());
            Box::pin(async move { response(model) })
        }));
    pump(&session).await;
    ...
    assert_eq!(
        seen.len(),
        3,
        "two ordered human turns plus one complete child batch"
    );
    ...
    for id in ["one", "two", "three"] {
        assert!(seen[2].contains(&format!("child evidence {id}")));
    }
    drop(seen);
    let entries = session.session_manager.lock().unwrap().get_entries();
    for id in ["one", "two", "three"] {
        assert_eq!(count_input(&entries, &format!("child evidence {id}")), 1);
    }
    let queue_metrics: Vec<_> = recorder
        .0
        .lock()
        .unwrap()
        .iter()
        .filter(|event| {
            event.operation
                == pi_agent_core::performance_metrics::PerformanceMetricOperation::SessionInput
        })
        .cloned()
        .collect();
    let classifications: Vec<_> = queue_metrics
        .iter()
        .map(|event| {
            event.measurements.as_ref().unwrap()[
...
async fn family_backlog_batches_without_combining_messages_or_overtaking_human_input() {
    ...
    assert_eq!(
        seen.len(),
        3,
        "two human turns then one family batch, not four extra model cycles"
    );
    ...
    let positions: Vec<_> = messages
        .iter()
        .map(|(id, _)| seen[2].find(id).unwrap())
        .collect();
    assert!(
        positions.windows(2).all(|pair| pair[0] < pair[1]),
        "corrections retain admission order"
    );
    drop(seen);
    let path = session
        .session_manager
    ...
    for (id, _) in messages {
        assert_eq!(count_input(&reopened.get_entries(), id), 1);
    }
    session.dispose_async(Some(false)).await;
}
    ...
    session.agent.set_stream_fn(Arc::new(move |model, _, _| {
        ...
        Box::pin(async move {
            ...
            assert!(
                ...
                "the deferred event queue was pinned before input delivery"
            );
            ...
            let saved = entries
                ...
                .find(|entry| {
                    entry.get("customType").and_then(Value::as_str)
                        == Some(RLM_CONTINUATION_STATE_CUSTOM_TYPE)
                })
            ...
            let previous = {
                ...
                session
                    .claim_rlm_parent_delivery(&mut ledger, &ids)
                    .unwrap()
            };
...
async fn failed_parent_task_ledger_write_does_not_publish_or_duplicate_task_state() {
    ...
    *session.session_manager.lock().unwrap() =
    ...
    let action = session.create_prepared_turn_action(
        ...
        None,
        Some(PreparedTurnActionOptions {
            custom_message: Some(family_message("ledger-failure", "parent")),
            ..Default::default()
        }),
    );
    let message = agent_message_from_delivery(&primary_delivery_record(&action).unwrap().message);
    ...
    assert!(
        error.contains("Parent-task ledger persistence failed"),
        "{error}"
    );
    assert_eq!(session.rlm_continuation.lock().unwrap().tasks, before.tasks);
    assert_eq!(
        *session.replied_to_parent_since_task.lock().unwrap(),
        replied_before
    );
    assert!(
        !session
    ...
    assert_eq!(
        ledgers.len(),
        1,
        "later event handling cannot register the same task twice"
    );
    ...
    *session.session_manager.lock().unwrap() =
    ...
    let parent_message = |id: &str| {
        let action = session.create_prepared_turn_action(
            ...
            None,
            Some(PreparedTurnActionOptions {
                custom_message: Some(family_message(id, "parent")),
                ..Default::default()
            }),
        );
    ...
    session
        .begin_rlm_parent_task(&parent_message("older-task"))
        .unwrap();
    ...
    assert!(
        held_through_append,
        "older snapshot released its ledger lock before the append and can overwrite newer state"
    );
    let path = session
        .session_manager
        .lock()
    ...
    assert_eq!(
        restored.tasks,
        session.rlm_continuation.lock().unwrap().tasks
    );
    session.dispose_async(Some(false)).await;
}
...
async fn automatic_family_batch_requires_matching_policies_and_known_agent_messages() {
    let (session, _root) = fixture().await;
    let create = |id: &str, relationship: &str| {
        session.create_prepared_turn_action(
            "steer",
            id,
            None,
            Some(PreparedTurnActionOptions {
                custom_message: Some(family_message(id, relationship)),
                ..Default::default()
            }),
        )
    };
    ...
    for relationship in ["parent", "sibling", "child"] {
        assert!(compatible_family_message_actions(
            &first,
    ...
    if let QueuedActionPayload::Turn(turn) = &mut next.payload {
        turn.execution_policy.run_before_agent_start =
            !turn.execution_policy.run_before_agent_start;
    }
    excluded.push(next);
    ...
    for next in excluded {
        assert!(
            !compatible_family_message_actions(&first, &next),
            "must not absorb {}",
            action_text(&next)
        );
    }
...
async fn family_batch_does_not_skip_a_contiguous_queue_barrier_or_replay_a_cancelled_message() {
    ...
    let (_, barrier) = enqueue(
        &session,
        "unknown-barrier",
        Some(family_message("unknown-barrier", "unknown")),
    );
    let (cancelled, cancelled_ticket) = enqueue(
        &session,
        "cancelled",
        Some(family_message("cancelled", "sibling")),
    );
    let (_, last) = enqueue(&session, "last", Some(family_message("last", "child")));
    session.cancel_session_actions(
        &|action| action.id == cancelled.id,
        "cancel queued fixture",
        None,
    );
    let contexts = Arc::new(Mutex::new(Vec::new()));
    let seen = contexts.clone();
    ...
    pump(&session).await;
    for ticket in [first, barrier, last] {
        ticket.ticket.completed.clone().await.unwrap();
    }
    ...
    assert_eq!(
        seen.len(),
        3,
        "do not scan past an incompatible queued message"
    );
    assert!(!seen[0].contains("unknown-barrier"));
    assert!(!seen[1].contains("child instruction last"));
    assert!(seen
        .iter()
        .all(|context| !context.contains("sibling instruction cancelled")));
    drop(seen);
    assert_eq!(
        count_input(
            &session.session_manager.lock().unwrap().get_entries(),
            "sibling instruction cancelled"
        ),
        0
    );
    session.dispose_async(Some(false)).await;
    ...
    for relationship in ["parent", "sibling", "child"] {
        tickets.push(
            enqueue(
                &session,
                relationship,
                Some(family_message(relationship, relationship)),
            )
            .1,
        );
    }
    ...
    for relationship in ["parent", "sibling", "child"] {
        assert!(
            count_input(
                ...
                &format!("{relationship} instruction {relationship}")
            ) <= 1
        );
    }
    session.dispose_async(Some(false)).await;
...
async fn cancelled_before_delivery_never_receives_durable_ack_or_transcript_entry() {
    let (session, _root) = fixture().await;
    let (action, ticket) = enqueue(
        &session,
        "cancel-before-delivery",
        Some(child_report("cancelled-report")),
    );
    let mut selected = session
        .action_store
        .lock()
...
async fn provider_retry_reuses_the_same_durable_input_without_duplicate_transcript_entries() {
    ...
    let reopened = SessionManager::open(&path, None, None).unwrap();
    assert_eq!(
        count_input(&reopened.get_entries(), "one input across provider retry"),
        1
    );
    session.dispose_async(Some(false)).await;
}
...
async fn new_dispatch_cannot_overtake_delayed_previous_assistant_or_tool_persistence() {
    ...
    let old_assistant = AssistantMessage {
        content: vec![ContentBlock::Text(TextContent::new(
            "older-assistant-entry",
        ))],
        ..Default::default()
    };
    let old_tool = pi_ai::types::ToolResultMessage::new(
        "older-call",
        "fixture",
    ...
    session.handle_agent_event(AgentEvent::MessageEnd {
        message: AgentMessage::Message(Message::Assistant(old_assistant)),
    });
    session.handle_agent_event(AgentEvent::MessageEnd {
        message: AgentMessage::Message(Message::ToolResult(old_tool)),
    });
    ...
    let position = |text: &str| {
        entries
            .iter()
            .position(|entry| entry.contains(text))
            .unwrap()
    };
