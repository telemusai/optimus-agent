...
async fn assert_attempt_metrics(
    ...
    let attempt_events: Vec<_> = events.iter().filter(|event| {
        event.operation == pi_agent_core::performance_metrics::PerformanceMetricOperation::ProviderAttempt
    }).collect();
    let terminal_events: Vec<_> = events.iter().filter(|event| {
        event.operation == pi_agent_core::performance_metrics::PerformanceMetricOperation::LogicalRequest
    }).collect();
    assert_eq!(attempt_events.len(), expected, "every provider attempt is measured");
    assert_eq!(terminal_events.len(), 1, "one request, including any retry, settles exactly once");
    let terminal = terminal_events[0];
    let group_id = terminal.correlation.as_ref().unwrap().logical_request_id.as_ref().unwrap();
    for (index, attempt) in attempt_events.iter().enumerate() {
        assert_eq!(attempt.correlation.as_ref().unwrap().logical_request_id.as_ref(), Some(group_id));
        assert_eq!(
...
async fn cancelled_retry_backoff_settles_one_cancelled_request_without_replay() {
    use pi_agent_core::performance_metrics::{PerformanceMetricMeasurement, PerformanceMetricOperation, PerformanceMetricOutcome};
    let session = post_compaction_continuation_tests::test_session_with_credentials().await;
    session.settings_manager.lock().unwrap().apply_overrides(
    ...
    session.agent.set_stream_fn(Arc::new(move |model, _, _| {
        ...
        Box::pin(async move {
            ...
            let failure = StreamFailureError::new("isolated retry cancellation fixture", StreamFailureInfo {
                kind: "server_error".into(), ..Default::default()
            });
            record_stream_failure(&model, &mut message, &ThrownStreamError::Failure(&failure));
            stream.push(AssistantMessageEvent::Error { reason: "error".into(), error: message });
            stream.end(None);
    ...
    let prompt = tokio::spawn(async move { prompt_session.prompt("cancel pending retry", None).await });
    tokio::time::timeout(std::time::Duration::from_secs(3), async {
        while session.retry_abort_controller.lock().unwrap().is_none() {
            tokio::task::yield_now().await;
        }
    }).await.expect("retry reaches cancellable backoff");
    assert!(!recorder.0.lock().unwrap().iter().any(|event| event.operation == PerformanceMetricOperation::LogicalRequest),
        "failed attempt stays unsettled while retry is pending");
    session.abort_retry();
    tokio::time::timeout(std::time::Duration::from_secs(3), prompt)
        .await.expect("prompt bounded").expect("prompt task joined").expect("prompt settled");
    tokio::time::timeout(std::time::Duration::from_secs(3), session.wait_for_idle())
        .await.expect("idle bounded").expect("idle settled");
    let events = recorder.0.lock().unwrap().clone();
    let terminals: Vec<_> = events.iter().filter(|event| event.operation == PerformanceMetricOperation::LogicalRequest).collect();
    assert_eq!(attempts.load(Ordering::SeqCst), 1, "cancellation must not replay the request");
    assert_eq!(terminals.len(), 1);
    assert_eq!(terminals[0].outcome, Some(PerformanceMetricOutcome::Cancelled));
    assert_eq!(terminals[0].measurements.as_ref().unwrap().get(&PerformanceMetricMeasurement::AttemptCount), Some(&Some(1.0)));
    session.dispose_async(Some(false)).await;
}
...
impl PerformanceMetricRecorder for RetryMetricRecorder {
    fn session_id(&self) -> &str { "retry-metric-fixture" }
    fn monotonic_now(&self) -> f64 { now_ms() }
    fn next_id(&self, scope: pi_agent_core::performance_metrics::PerformanceMetricIdScope) -> String {
...
async fn retry_group_settles_one_terminal_and_later_turns_start_a_new_group() {
    ...
    for prompt in ["retry group turn one", "later turn two"] {
        tokio::time::timeout(std::time::Duration::from_secs(8), session.prompt(prompt, None))
            .await.expect("prompt bounded").expect("prompt settled");
        tokio::time::timeout(std::time::Duration::from_secs(8), session.wait_for_idle())
            .await.expect("idle bounded").expect("idle settled");
    }
    ...
    let logical_id = |event: &PerformanceMetricEvent| {
        event.correlation.as_ref().and_then(|correlation| correlation.logical_request_id.clone())
    };
    let provider_attempts: Vec<_> = events.iter().filter(|event| {
        event.operation == pi_agent_core::performance_metrics::PerformanceMetricOperation::ProviderAttempt
    }).collect();
    assert_eq!(provider_attempts.len(), 3, "failure, successful retry, then later prompt");
    let retry_id = logical_id(provider_attempts[0]).expect("initial request id");
    assert_eq!(logical_id(provider_attempts[1]).as_deref(), Some(retry_id.as_str()));
    ...
    for (index, event) in events.iter().enumerate() {
        if event.operation != pi_agent_core::performance_metrics::PerformanceMetricOperation::LogicalRequest {
            continue;
        }
        let Some(id) = logical_id(event) else { continue };
        assert!(
            terminals.insert(id.clone(), index).is_none(),
            "a logical request must settle at most one terminal, but {id} settled twice"
        );
    }
    assert_eq!(terminals.len(), 2, "the failed attempt must not prematurely settle a separate group");
    for (id, expected_count) in [(&retry_id, 2.0), (&later_id, 1.0)] {
        let terminal = &events[*terminals.get(id).expect("each group has a terminal")];
        assert_eq!(terminal.outcome, Some(pi_agent_core::performance_metrics::PerformanceMetricOutcome::Success));
        assert_eq!(
            terminal.measurements.as_ref().unwrap().get(
                &pi_agent_core::performance_metrics::PerformanceMetricMeasurement::AttemptCount,
            ),
    ...
    for (index, event) in events.iter().enumerate() {
        if event.operation != pi_agent_core::performance_metrics::PerformanceMetricOperation::ProviderAttempt {
            continue;
        }
        let Some(id) = logical_id(event) else { continue };
        let Some(terminal) = terminals.get(&id) else { continue };
        assert!(
            index < *terminal,
